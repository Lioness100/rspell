...
export const updateFutureIssues = (issue: Issue, replacer: string, issues: Issue[]) => {
	...
	issue.line.text = issue.line.text.slice(0, issue.col - 1) + replacer + issue.line.text.slice(issue.col + issue.text.length - 1);
...
export const handleIssue = async (issues: (Issue & { resolved?: boolean })[], issue: Issue & { resolved?: boolean }) => {
	...
	if (action === Action.ReplaceAll || action === Action.IgnoreAll || action === Action.SkipFile) {
		for (const futureIssue of [...issues]) {
			if (action === Action.SkipFile ? futureIssue.uri !== issue.uri : futureIssue.text.toLowerCase() !== issue.text.toLowerCase()) {
				continue;
			}
